...
import { createStage, frameCamera, setStageTheme } from "./scene.js";
...
export class Viewer {
  constructor(el, opts = {}) {
    ...
    const stage = createStage(el, this.theme);
    this.stage = stage;
  ...
  setPuzzle(puzzle) {
    ...
    this.board = new Board(puzzle, this.theme);
  ...
  setTheme(theme) {
    ...
    if (this.board) {
      ...
      if (this.carried) this.board.applyObjectTheme(this.carried.group);
    }
  }
  ...
  dispose() {
    ...
    this.hud.dispose();
    if (this.stage.stars) {
      this.stage.stars.geometry.dispose();
      this.stage.stars.material.dispose();
    }
