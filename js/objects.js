...
export function buildWall() {
  ...
  const stone = new THREE.MeshStandardMaterial({ color: 0x8b94a0, roughness: 0.95, flatShading: true });
  const light = new THREE.MeshStandardMaterial({ color: 0xa9b2bd, roughness: 0.95, flatShading: true });
...
export function buildDoor(color, state, orientation = "z") {
  const g = new THREE.Group();
  const frameMat = new THREE.MeshStandardMaterial({ color: 0x6b7480, roughness: 0.9, flatShading: true });
...
export function buildObject(obj, orientation) {
  ...
  switch (obj.type) {
    ...
    case "door":
      return buildDoor(color, obj.state, orientation);
    default:
