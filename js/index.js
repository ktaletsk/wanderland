...
import { detectTheme, watchTheme } from "./theme.js";

function render({ model, el }) {
  el.classList.add("wanderland-pg");
  const initialTheme = detectTheme(el, model);
  el.dataset.wanderlandTheme = initialTheme;
  const viewer = new Viewer(el, { character: model.get("character") || "mo", theme: initialTheme });
  const stopTheme = watchTheme(el, model, (theme) => {
    el.dataset.wanderlandTheme = theme;
