...
export class Board {
  constructor(puzzle, theme = "light") {
    this.puzzle = puzzle;
    this.theme = theme;
  ...
  _build() {
    const p = this.puzzle;
    const colors = colorsForTheme(this.theme);
    ...
    const lavaMat = new THREE.MeshStandardMaterial({ color: colors.lava, emissive: colors.lavaEmissive, emissiveIntensity: 0.7, roughness: 0.6 });
    this._grassMat = grassMat;
    ...
    const water = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: colors.water, roughness: 0.4, metalness: 0.1, transparent: true, opacity: colors.waterOpacity })
    );
    this._waterMat = water.material;
  ...
  _addObject(c, r, obj) {
    ...
    const group = buildObject(obj, orientation, colors);
    const pos = this.worldPos(c, r);
  ...
  setTheme(theme) {
    ...
    if (this._grassMat) this._grassMat.color.set(colors.grass);
    if (this._dirtMat) this._dirtMat.color.set(colors.dirt);
    if (this._lavaMat) {
      this._lavaMat.color.set(colors.lava);
      this._lavaMat.emissive.set(colors.lavaEmissive);
    }
    if (this._waterMat) {
      this._waterMat.color.set(colors.water);
      this._waterMat.opacity = colors.waterOpacity;
    }
    for (const gem of this.gems.values()) {
      gem.mat.color.set(colors.gem);
      gem.mat.emissive.set(colors.gemEmissive);
    }
    if (this.goalRing) {
      this.goalRing.material.color.set(colors.goal);
      this.goalRing.material.emissive.set(colors.goal);
    }
    for (const entry of this.objects.values()) this.applyObjectTheme(entry.group);
  }

  applyObjectTheme(group) {
    applyObjectTheme(group, colorsForTheme(this.theme));
  }
...
function applyObjectTheme(group, colors) {
  group.traverse((child) => {
    if (!child.material) return;
    const mats = Array.isArray(child.material) ? child.material : [child.material];
    for (const mat of mats) {
      const role = mat.userData && mat.userData.themeRole;
      if (role && colors[role] != null) mat.color.set(colors[role]);
    }
  });
}
